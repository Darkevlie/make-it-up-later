...
class DataBase
{
    ...
    public:
                ...
                if (query.next()) {
                    ...
                    if (status.isEmpty()) {
                        ...
                        if (query.exec()) {
                            ...
                            return "auth_err"; // Ошибка при установке статуса
                        }
                    } else {
                        return "auth_err"; // Пользователь уже зареган
                    }
                } else {
                    return "auth_false"; // Пользователь не найден
                }
            } else {
                return "auth_err"; // Ошибка при выполнении запроса
            }
        }
                ...
                if (query.next()) {
                    ...
                    if (startTime.isEmpty()) {
                        ...
                        if (query.exec()) {
                            ...
                            return "start_err"; // Ошибка при установке времени начала
                        }
                    } else {
                        ...
                        if (query.exec()) {
                            ...
                            return "start_err"; // Ошибка при обновлении времени начала
                        }
                    }
                } else {
                    return "start_err"; // Пользователь с заданным дескриптором соединения не найден
                }
            } else {
                return "start_err"; // Ошибка при выполнении запроса
            }
        }
        ...
        static QByteArray DataBase::finish(const QString &time, const QString &connectionDescriptor) {
            ...
            if (query.exec()) {
                if (query.next()) {
                    ...
                    if (startTime.isEmpty()) {
                        return "finish_reerr"; // Поле начато пусто
                    } else {
                        ...
                        if (query.exec()) {
                            ...
                            return "finish_err"; // Ошибка при обновлении статистики
                        }
                    }
                } else {
                    return "finish_err"; // Пользователь с заданным дескриптором соединения не найден
                }
            } else {
                return "finish_err"; // Ошибка при выполнении запроса
            }
        }
        ...
        static QByteArray DataBase::registerUser(const QString &login, const QString &password, const QString &email) {
            ...
            if (query.exec()) {
                if (query.next()) {
                    return "reg_err"; // Пользователь с таким логином уже существует
                } else {
                    ...
                    if (query.exec()) {
                        return "reg_ok"; // Пользователь успешно зарегистрирован
                    } else {
                        return "reg_err"; // Ошибка при выполнении запроса
                    }
                }
            } else {
                return "reg_err"; // Ошибка при выполнении запроса
